...
void BinaryTree::PrintAt(Node *n, int xParent, int yParent)
{
	...
	MyRectangle box;
	if (n == root) {
		box.DrawSkelaton(n->x - 3, n->y - 3, BOXWIDTH, BOXHEIGHT, value, 10);
		pars.push_back(n->getValue());
	}
	else if (n->r == true) {
		box.DrawSkelaton(n->x - 3, n->y - 3, BOXWIDTH, BOXHEIGHT, value, 4);
		pcont += 1;
		pars.push_back(n->getValue());
		chils.push_back(n->getValue());
		
	}
	else if (n->yel == true) {
		box.DrawSkelaton(n->x - 3, n->y - 3, BOXWIDTH, BOXHEIGHT, value, 6);
		lcont += 1;
		chils.push_back(n->getValue());
	}
